...
class Uploadify extends CInputWidget 
{
	private $body = null;
    ...
    */
	public function run() 
	{
        ...
        $this->baseUrl = Yii::app()->getAssetManager()->publish(dirname(__FILE__).DIRECTORY_SEPARATOR.'skin');
		
        $clientScript->registerScriptFile($this->baseUrl . '/jquery.uploadify.v2.1.0.min.js', CClientScript::POS_HEAD);
		$clientScript->registerScriptFile($this->baseUrl . '/swfobject.js', CClientScript::POS_HEAD); 
		$clientScript->registerCssFile($this->baseUrl . '/uploadify.css');
...
EOP;
        ...
        $this->htmlOptions['data-no-uniform'] = 'true';
        ...
        if (!empty($validCallbacks)) {
            foreach ($value as $key=>$val) {
                if (!in_array($key, $validCallbacks)) {
                    throw new CException(Yii::t('Uploadify', '{k} must be one of: {c}', array('{k}'=>$key, '{c}'=>implode(', ', $validCallbacks))));
                }
            }
        }
    }
        ...
        if (!empty($validOptions)) { 
            foreach ($value as $key=>$val) {
                ...
                if (!array_key_exists($key, $validOptions)) {
                    throw new CException(Yii::t('Uploadify', '{k} is not a valid option', array('{k}'=>$key)));
                }
                $type = gettype($val);    
                if ((!is_array($validOptions[$key]['type']) && ($type != $validOptions[$key]['type'])) || (is_array($validOptions[$key]['type']) && !in_array($type, $validOptions[$key]['type']))) {
                        throw new CException(Yii::t('Uploadify', '{k} must be of type {t}', 
                ...
                if (array_key_exists('possibleValues', $validOptions[$key])) {
                   if (!in_array($val, $validOptions[$key]['possibleValues'])) {
                        throw new CException(Yii::t('Uploadify', '{k} must be one of: {v}', array('{k}'=>$key, '{v}'=>implode(', ', $validOptions[$key]['possibleValues']))));
                   }
                }
    ...
    */
    ...
    {
        if (!is_array($value))
            throw new CException(Yii::t('Uploadify', 'callbacks must be an associative array'));
    ...
    */
    ...
    {
        if (!is_array($value))
            throw new CException(Yii::t('Uploadify', 'options must be an array'));
    ...
    */
    protected function makeOptions()
    {
        ...
        if(!array_key_exists('uploader', $this->options))
            $this->options['uploader'] = $this->baseUrl . '/uploadify.swf?PHPSESSID=' . session_id();
        if(!array_key_exists('cancelImg', $this->options))
            $this->options['cancelImg'] = $this->baseUrl . '/cancel.png';
