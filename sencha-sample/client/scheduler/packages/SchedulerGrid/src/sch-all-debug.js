/*
   ...
   THIS IS A PRIVATE BUILD FOR INTERNAL EVALUATION WITH SENCHA ARCHITECT.
...
Ext.define('Sch.mixin.Localizable', {
    ...
    /**
     * @prop {Object} l10n Container of locales for the class.
     */
    l10n                : {},
     ...
     */
    locale          : function (id, legacyHolderProp) {
        var prop = legacyHolderProp || this.legacyHolderProp;

        if (this.legacyMode) {
            var holder      = prop ? this[prop] : this;
            var holderProto;
            if (this.self && this.self.prototype) {
                holderProto = prop ? this.self.prototype[prop] : this.self.prototype;
            }

            if (holder) {
                if (holder.hasOwnProperty(id)) return holder[id];
                if (holderProto && holderProto.hasOwnProperty(id)) return holderProto[id];
            }
        }
    ...
    mapLegacyLocales : function (locales, fromHolder) {
        fromHolder = fromHolder || this;
        for (var i = 0, l = locales.length, locale; i < l; i++) {
            locale = locales[i];
            if (fromHolder.hasOwnProperty(locale)) this.l10n[locale] = fromHolder[locale];
        }
    }
});
...
Ext.define("Sch.feature.PointDragZone", {
    ...
    onDragDrop: function(e, id){
        ...
        if (dragData.valid && dragData.startDate && dragData.endDate) {
            ...
            if (doFinalize && me.isValidDrop(dragData.resourceRecord, dragData.newResource, dragData.relatedEventRecords)) 
            {
    ...
    finalize : function(updateRecords) {
        ...
        if(updateRecords){ 
            ...
            s.fireEvent('aftereventdrop', s, dragData.eventRecords);
        } else {
 ...
 * @extends Sch.feature.PointDragZone
 * A custom scheduler dragzone that also acts as the dropzone, and optionally constrains the drag to the resource area that contains the dragged element.
 * @constructor
 * @param {Object} config The object containing the configuration of this model.
 */
Ext.define("Sch.feature.SchedulerDragZone", {
    ...
    updateRecords : function(context) {
        var me              = this,
            ...
            draggedRecords  = context.eventRecords,
            draggedEvent    = draggedRecords[0];

        // Process dragged record
        draggedEvent.beginEdit();
        if (newResource !== context.resourceRecord) {
            draggedEvent.unassign(context.resourceRecord);
        ...
        var start               = context.startDate,
            relatedEventRecords = context.relatedEventRecords,
            timeDiff            = start - context.origStart,
            indexDiff,
            isTreeStore         = Ext.data.TreeStore && resourceStore instanceof Ext.data.TreeStore;

        if (isTreeStore) {
            indexDiff = s.indexOf(context.resourceRecord) - s.indexOf(newResource);
        } else {
            indexDiff = resourceStore.indexOf(context.resourceRecord) - resourceStore.indexOf(newResource);
        }
        ...
        s.fireEvent('eventdrop', s, context.eventRecords, false);
    },
    ...
    updateDragContext : function(e) {
        ...
        dd.newResource = s.constrainDragToResource ? dd.resourceRecord : this.resolveResource([proxyRegion.left + dd.offsets[0], proxyRegion.top + dd.offsets[1]], e);
...
Ext.define("Sch.feature.DragDrop", {
    ...
    initProxyLessDD : function() {
        ...
        s.eventDragZone = Ext.create("Sch.feature.SchedulerDragZone", s.ownerCt.el, Ext.apply({
            ddGroup             : s.id,
            schedulerView       : s,
...
Ext.define("Sch.view.model.TimeAxis", {
    ...
    */
    ...
    /**
     *  Gets the position of a date on the projected time axis
...
Ext.define('Sch.model.Customizable', {
    ...
    // @OVERRIDE
    // Overridden to be able to clear the previous record field values. Must be done here for have access to 'previous' after
    // an endEdit call.
    afterEdit : function() {
        this.callParent(arguments);
...
Ext.define("Sch.data.EventStore", {
     ...
     * @param {Sch.model.Event} record The record to append the store
     */
    append : function(record) {
        this.add(record);
    }
...
and the functionality of this mixin is not related to the standard Ext JS store filtering (which utilizes Ext.util.Filter etc). This implementation however should be flexible
...
Filtering of a tree store is different from filtering of flat stores. In flat stores all nodes (items) are of the same type and on the same hieararchy level.
...
//            var stretcher = view.getEl().down('.x-stretcher');
//
...
//            if (stretcher.dom.clientHeight) {
//                region = stretcher.getRegion();
//            }
//        }
...
Ext.define("Sch.view.Vertical", {
    ...
    translateToScheduleCoordinate: function (y) {
        var view = this.view;
        return y - view.el.getY() + view.el.getScroll().top;
    },
    ...
    getScheduleRegion: function (resourceRecord, eventRecord) {
        var view        = this.view,
            region      = resourceRecord ? Ext.fly(view.getCellByPosition({ column : view.resourceStore.indexOf(resourceRecord), row : 0})).getRegion() : view.getTableRegion(),
    ...
    getResourceColumnWidth : function(resource) {
        var view    = this.view,
            index   = view.resourceStore.indexOf(resource),
            column  = view.headerCt.getGridColumns()[index];

        return column.rendered ? column.getWidth() : column.width;
    },
    ...
    */
    ...
    resolveResource: function (el) {
        var view = this.view;
        el = Ext.fly(el).is(view.cellSelector) ? el : Ext.fly(el).up(view.cellSelector);

        if (el) {
            var hd = view.getHeaderByCell(el.dom ? el.dom : el);

            if (hd) {
                return view.resourceStore.getAt(view.headerCt.getHeaderIndex(hd));
            }
        }
    ...
    onEventAdd: function (s, recs) {
        ...
        if (recs.length === 1) {
            ...
            view.repaintEventsForResource(view.store.first());
        }
    },
    ...
    renderSingle : function (event) {
        ...
        var containerCell   = view.getCell(view.store.getAt(0), view.headerCt.getHeaderAtIndex(rIndex)).first();
    ...
    */
    getTimeSpanRegion: function (startDate, endDate) {
        var view        = this.view,
            ...
            endY        = view.getCoordinateFromDate(endDate || startDate),
            tableRegion = view.getTableRegion(),
            width       = tableRegion ? tableRegion.right - tableRegion.left : view.el.dom.clientWidth; // fallback in case grid is not rendered (no rows/table)
    ...
    */
    ...
    setColumnWidth : function (width, preventRefresh) {
        ...
        if (view.panel) {
            view.panel.resourceColumnWidth = width;
        }
...
Plugin for printing an Ext Scheduler instance. Please note that this will not generate a picture perfect
...
Ext.define('Sch.plugin.Export', {
    ...
    */
    doExport: function (conf, callback, errback) {
        ...
        var format, htmlArray, calculatedPages, params;
    ...
    */
    resizePanelHTML: function (HTML) {
        ...
        if (Ext.isIE6 || Ext.isIE7 || Ext.isIEQuirks){
            ...
            lockedEl = lockedGrid.body.child('.x-grid-view') ? lockedGrid.body.child('.x-grid-view') :
                    lockedGrid.body.child('.x-tree-view');
            ...
            lockedEl = lockedGrid.body.child('.x-grid-view') || lockedGrid.body.child('.x-tree-view');
    ...
    */
    showRows: function () {
        this.scheduler.getEl().select('.x-grid-row').each(function(el){
...
Ext.define("Sch.plugin.TreeCellEditing", {
    ...
    */
    ...
    onBeforeCellEdit : function(me, context) {
        ...
        if (!column.dataIndex && column.field && context.value === undefined) {
            context.value = column.field.getDisplayValue(context.record);
        }
    },
    ...
    // @OVERRIDE - Fixes layout issues during editing IE
...
To use this feature, assign an {@link Sch.data.EventStore eventStore} to the {@link Sch.mixin.SchedulerPanel#cfg-resourceZones resourceZones}  config on the main Scheduler panel class.
...
//    barMargin           : null,
    ...
    */
    getFormattedEndDate: function (endDate, startDate) {
        ...
        // If time is midnight, and end date is greater then start date then format the date inclusively as 'the whole previous day'.
    ...
    */
    ...
    /**
    * Gets the date for an XY coordinate
    * @param {Int} coordinate The page X or Y coordinate
    * @param {String} roundingMethod The rounding method to use
    * @param {Boolean} local, true if the coordinate is local to the scheduler view element 
    * @returns {Date} the Date corresponding to the xy coordinate
    */
    ...
    /**
    * Gets the date for an Y coordinate
    * @param {Int} x The page Y coordinate
    * @param {String} roundingMethod The rounding method to use
...
Ext.define("Sch.mixin.TimelineView", {
    ...
    */
    ...
    refreshKeepingScroll : function() {
        ...
        this.refresh();
        if (this.up('tablepanel[lockable=true]').lockedGridDependsOnSchedule) this.lockingPartner.refresh();
    ...
    getTableRegion: function () {
        var tableEl = this.el.down('.x-grid-table');
...
Ext.define('Sch.mixin.AbstractSchedulerView', {
    ...
    */
    ...
    eventTpl: [
        '<tpl for=".">',
            '<div unselectable="on" id="{{evt-prefix}}{id}" style="right:{right}px;left:{left}px;top:{top}px;height:{height}px;width:{width}px;{style}" class="sch-event x-unselectable {internalCls} {cls}">',
    ...
    */
    ...
    inheritables: function () {
        return {
            ...
            repaintEventsForResource : function(resourceRecord) {
                var index = this.store.indexOf(resourceRecord);
            ...
            handleScheduleEvent : function(e) {
                ...
                if (t) {
                    ...
                    if (this.orientation == 'horizontal') {
                        ...
                        var cellNode    = e.getTarget(this.cellSelector, 5);
...
Ext.define('Sch.mixin.Zoomable', {
    ...
    */
     ...
     * @param {Boolean} preserveSpan True to keep current time frame.
     *
     * @return {Int} level Current zoom level or null if it hasn't changed.
     */
    zoomToLevel: function (level, preserveSpan) {
        ...
        var optimalDates            = preserveSpan ? {} : this.calculateOptimalDateRange(centerDate, panelSize, nextZoomLevel);
     ...
     * @param {Boolean} adjustStartDate If `true` then will set start date to one time frame unit earlier than `span.start` value.
     * Use this to have some margin for comfortable reading.
     *
     * @return {Int} level Current zoom level or null if it hasn't changed.
     */
    zoomToSpan : function (span, adjustStartDate) {
        if (span.start && span.end && span.start < span.end) {

            // generate ticks for the new time frame
            var ticks       = this.timeAxis.generateTicks(adjustStartDate ? Sch.util.Date.add(span.start, this.timeAxis.mainUnit, -1) : span.start, span.end, this.timeAxis.unit, this.timeAxis.increment);
            ...
            var currLevel   = Math.floor(this.getCurrentZoomLevelIndex()),
                ...
                timeAxis    = Ext.create('Sch.data.TimeAxis');


            var zoomLevel, preset, headerConfig, bottom, middle, unit, zoomed = null;
            ...
            while (level >= 0 && level <= this.zoomLevels.length - 1) {
                ...
                bottom      = preset.headerConfig.bottom;
                middle      = preset.headerConfig.middle;
                ...
                // build timeAxis ticks
                ticks = timeAxis.generateTicks(adjustStartDate ? Sch.util.Date.add(span.start, timeAxis.mainUnit, -1) : span.start, span.end, timeAxis.unit, timeAxis.increment);

                // if zooming out
                if (inc < 0) {
                    // if new column width fit zoom level width
                    if (width / ticks.length >= zoomLevel.width) {
                        zoomed = this.zoomToLevel(level, true);
                        break;
                    }
                // if zooming in
                } else {
                    // if col width doesn't fit zoom level width
                    if (width / ticks.length < zoomLevel.width) {
                        // let's apply previous zoom level (if it's not currently active one)
                        if (currLevel !== level - inc) {
                            zoomed = this.zoomToLevel(level - inc, true);
                            break;
                        }
                        break;
                    }
            ...
            // apply the new timeframe
            this.setTimeSpan(adjustStartDate ? Sch.util.Date.add(span.start, this.timeAxis.mainUnit, -1) : span.start, span.end);
...
Ext.define('Sch.mixin.AbstractTimelinePanel', {
    ...
    * @cfg {String} viewPreset
     * A key used to lookup a predefined {@link Sch.preset.ViewPreset} (e.g. 'weekAndDay', 'hourAndDay'), managed by {@link Sch.preset.Manager}. See Sch.preset.Manager for more information.
    * @property viewPreset
     */
    ...
    */
    ...
    onNormalGridCollapse : function() {
        ...
        if (!me.lockedGrid.rendered) {
            ...
            var height = me.lockedGrid.headerCt && me.lockedGrid.headerCt.getHeight();
...
Ext.define('Sch.mixin.SchedulerPanel', {
    ...
    */
    ...
    /**
    * @cfg {Sch.data.EventStore} resourceZones A special store containing data used to highlight the underlying schedule for the resources. This can be used to color non-working time or any other meta data associated with a resource.
    */
    resourceZones : null,
        ...
        return {
            ...
            initComponent: function () {
                ...
                */
                ...
                // (it is disabled by default in the Lockable mixin)
    ...
    */
    ...
    setOrientation: function (orientation, force) {
        ...
        if (orientation === 'horizontal') {
            ...
            me.reconfigureLockable(me.resourceStore, me.horizontalColumns);
        } else {
            ...
            me.reconfigureLockable(me.timeAxis, me.verticalColumns.concat(me.createResourceColumns()));
...
Ext.define('Sch.locale.Locale', {
    ...
    constructor : function () {
        ...
        var oldRequire = Ext.Loader.require;
        var me = this;

        // patch Ext.Loader.require() to call this.apply() for each loaded class
        Ext.Loader.require = function(expressions, fn, scope, excludes) {
            // here we just provide own callback to process
            // the class after it get loaded
            // "function(foo)" is used because callback must have some argument,
            // only then ExtJs provides loaded classes to it
            oldRequire.call(Ext.Loader, expressions, function(foo) {
                // trying to apply locales for the loaded class
                // (converting arguments object to array using slice)
                me.apply.call(me, Array.prototype.slice.call(arguments, 0));
                // start original callback ..if any
        ...
        if (this.l10n) {
            ...
            if (classNames) {
                ...
                for (var i = 0, l = classNames.length; i < l; i++) {
                    ...
                    if (name && name in this.l10n) {
                        ...
                        if (cls) {
                            Ext.override(cls, { l10n : this.l10n[name] });
                        }
                    }
                }
            ...
            // localize all the classes that we know about
                ...
                for (var clsName in this.l10n) {
                    ...
                    if (cls) {
                        Ext.override(cls, { l10n : this.l10n[clsName] });
                    }
                }
            }
        }
