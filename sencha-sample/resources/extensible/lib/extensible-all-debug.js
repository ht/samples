 ...
 * @singleton
 */
var strpDepartmentLoading = false;
var strpGroupLoading = false;
            ...
            if (!this.eventTpl) {
                this.eventTpl = !(Ext.isIE || Ext.isOpera) ?
                    Ext.create('Ext.XTemplate',
                        '<div data-qtip="<b>{TitleToolTip}</b><br/>{Notes}<br><b>{Staff}</b>" id="{_elId}" class="{_extraCls} ext-cal-evt ext-cal-evr" style="left: {_left}%; width: {_width}%; top: {_top}px; height: {_height}px;">',
                    ...
                    : Ext.create('Ext.XTemplate',
                    '<div data-qtip="<b>{TitleToolTip}</b><br/>{Notes}<br><b>{Staff}</b>" id="{_elId}" class="ext-cal-evt {_extraCls}" style="left: {_left}%; width: {_width}%; top: {_top}px;">',
        ...
        getEventTemplate: function () {
            if (!this.eventTpl) {
                ...
                tpl = !(Ext.isIE || Ext.isOpera) ?
                    Ext.create('Ext.XTemplate',
                        '<div data-qtip="<b>{TitleToolTip}</b><br/>{Notes}<br><b>{Staff}</b>" class="{_extraCls} {spanCls} ext-cal-evt ext-cal-evr">',
                        body,
                        '</div>'
                    )
                    : Ext.create('Ext.XTemplate',
                    '<tpl if="_renderAsAllDay">',
                    '<div data-qtip="<b>{TitleToolTip}</b><br/>{Notes}<br><b>{Staff}</b>" class="{_extraCls} {spanCls} ext-cal-evt ext-cal-evo">',
                    '<div class="ext-cal-evm">',
                    '<div class="ext-cal-evi">',
                    '</tpl>',
                    '<tpl if="!_renderAsAllDay">',
                    '<div data-qtip="<b>{TitleToolTip}</b><br{Notes}<br><b>{Staff}</b>" class="{_extraCls} ext-cal-evt ext-cal-evr">',
                    '</tpl>',
                    body,
                    '<tpl if="_renderAsAllDay">',
                    '</div>',
...
//DoHuuHai: Bo sung các column con thiếu
Extensible.calendar.data.EventMappings = {
    EventId: {
        name: 'EventId',
        mapping: 'id',
...
Ext.define('Extensible.calendar.template.DayBody', {
        ...
        constructor: function (config) {
            ...
            Extensible.calendar.template.DayBody.superclass.constructor.call(this,
                ...
                '<tpl for="times">',
                '<div class="ext-cal-bg-row ext-row-{[xindex]}" style="height:{parent.hourHeight}px;">',
                '<div class="ext-cal-bg-row-div {parent.hourSeparatorCls}" style="height:{parent.hourSeparatorHeight*2}px;"></div>',
                '</div>',
                '</tpl>',
                '</div>',
                '</div>',
                ...
                '<tpl for="times">',
                '<div class="ext-cal-bg-row ext-row-{[xindex]}" style="height:{parent.hourHeight}px;">',
                '<div class="ext-cal-bg-row-div {parent.hourSeparatorCls}" style="height:{parent.hourSeparatorHeight*3}px;"></div>',
                '</div>',
                '</tpl>',
                '</div>',
                '</div>',
                ...
                '<tpl for="times">',
                '<div class="ext-cal-bg-row ext-row-{[xindex]}" style="height:{parent.hourHeight}px;">',
                '<div class="ext-cal-bg-row-div {parent.hourSeparatorCls}" style="height:{parent.hourSeparatorHeight*4}px;"></div>',
                '</div>',
                '</tpl>',
                '</div>',
                '</div>',
                ...
                '<tpl for="times">',
                '<div class="ext-cal-bg-row ext-row-{[xindex]}" style="height:{parent.hourHeight}px;">',
                '<div class="ext-cal-bg-row-div {parent.hourSeparatorCls}" style="height:{parent.hourSeparatorHeight*5}px;"></div>',
                '</div>',
                '</tpl>',
                '</div>',
                '</div>',
        ...
        applyTemplate: function (o) {
            ...
            var numberSegment = 60 / this.ddIncrement;
            templateConfig = {
                days: days,
                ...
                dayHeight: dayHeight,
                hourSeparatorHeight: (this.hourHeight / numberSegment)
            };
...
/*
 ...
 Ext.define('Extensible.calendar.form.field.GroupText', {
 extend: 'Ext.form.field.Text',
 alias: 'widget.extensible.grouptext'
 });*/

/**
 * @class Extensible.form.field.ComboRange
 * @extends Ext.form.Field
 * A combination field that includes plan type and room and editor, as well as an optional all-day checkbox.
 * @constructor
 * @param {Object} config The config object
 */
Ext.define('Extensible.form.field.TreeDepartmentGroupRange', {
    extend: 'Ext.form.FieldContainer',
    alias: 'widget.extensible.treedepartmentgrouprangefield',
    ...
    initComponent: function () {
        var me = this;//console.log(Global.authData.userEmpInfo);return;
        me.layout = me.fieldLayout;
        MyApp.util.Utilities.userScheduleName = Global.authData.userEmpInfo.ACEMP_NAME;
        MyApp.util.Utilities.userScheduleID = Global.authData.userEmpInfo.ACEMP_ID * 1;
    ...
    getFieldConfigs: function () {
        var me = this;
        //console.log('Edit vao day');
        return [
            me.getTreeDepartmentConfig(-1, 1),
            me.getGroupNameConfig(),
            me.getTreeGroupConfig()
        ];
    },

    getDepartments: function () {
        return {
            xtype: 'extensible.departmentcombo',
            labelWidth: 30,
            name: 'department',
            listeners: {
                'select': {
                    fn: function () {
                        this.onDepartmentComboChange();
                    },
                    scope: this
                }
            }
    ...
    onDepartmentComboChange: function () {
        var checkOnlyActive = 0;
        if (Ext.getCmp('projectJobCheckbox').getValue())
            checkOnlyActive = 1;
        //console.log(checkOnlyActive);
        if (strpDepartmentLoading == false) {
            strpDepartmentLoading = true;
            Ext.getStore('strpDepartment').load({
                params: {
                    sectionId: Ext.getCmp('cboScheduleDepartment').getValue(),
                    chkOnlyActive: checkOnlyActive
                },
                callback: function (a, b, c) {
                    if (c) {
                        strpDepartmentLoading = false;
                    }
                }
            });
        }
    },
    getIsExecutingConfig: function () {
        return {
            xtype: 'checkboxfield',
            id: 'projectJobCheckbox',
            boxLabel: '進行中のみ',
            //Son 6666
            handler: this.onExcutingChangeDepartment,
            checked: true,
            scope: this
        };
    },
    //process for checkbox of department
    onExcutingChangeDepartment: function () {
        //console.log('toi day');
        var checkbox = Ext.getCmp('projectJobCheckbox');
    ...
    getTreeDepartmentConfig: function (section_id, check_only_active) {
        var me = this;
        var treeStore = Ext.create('Ext.data.TreeStore', {
            autoLoad: false,
            id: 'strpDepartment',
            itemId: 'strpDepartmentItem',
            storeId: 'strpDepartment',
            proxy: {
                type: 'rest',
                url: apiUrl + 'schedule/GetProjectBySectionExtra',
                extraParams: {
                    sectionId: section_id,
                    chkOnlyActive: check_only_active
                },
                reader: {
                    type: 'json'
                },
        ...
        var trpDepartment = Ext.create('Ext.tree.Panel', {
            id: 'trpDepartment',
            itemId: 'trpDepartmentItem',
            width: 300,
            height: 250,
            store: treeStore,
            ...
            listeners: {
                itemclick: {
                    fn: function (view, record, item, index, event) {
                        if ('root' != record.data.parentId) {//console.log(record);
                            Ext.getCmp('txtScheduleDepartment').setValue(record.parentNode.data.text + '/' + record.data.text);
                            Ext.getCmp('txtScheduleDepartmentHidden').setValue(record.data.id);
                        }
                    },
                    scope: this
                }
            ...
            items: [
                {
                    ...
                    items: [
                        me.getDepartments(),
                        me.getGroupNameConfigs(),
                        me.getIsExecutingConfig()
                    ]
                },
                {
                    xtype: 'container',
    ...
    getGroupNameConfig: function () {
        return {
            xtype: 'label',
            id: this.id + '-departments-label',
            text: "",
            width: 5
        };
    },

    getGroupNameConfigs: function () {
        return {
            xtype: 'label',
            id: this.id + '-departments2-label',
            text: "",
            width: 10
        };
    },
    ...
    getDepartment: function () {
        return {
            xtype: 'textfield',
            id: 'txtScheduleDepartment',
            itemId: this.id + '-department-text-item',
            width: 300,
            readOnly: true,
            labelWidth: 100,
    ...
    getDepartmentHidden: function () {
        return {
            xtype: 'textfield',
            id: 'txtScheduleDepartmentHidden',
            //id: this.id + '-department-text-id-hidden',
            itemId: this.id + '-department-text-item-hidden',
            width: 300,
            hidden: true,
            readOnly: true,
            labelWidth: 100,
    ...
    getSpaceLine: function () {
        return {
            xtype: 'extensible.spaceline',
            itemId: this.id + '-spacelinefield',
            anchor: '100%'
        };
    },

    getSpaceLine2: function () {
        return {
            xtype: 'extensible.spaceline',
            itemId: this.id + '-spacelinefield2',
            anchor: '100%'
        };
    },
    //Cay thu 2
    getTreeGroupConfig: function () {
        var me = this;
        var groupTreeStore = Ext.create('Ext.data.TreeStore', {
            autoLoad: false,
            id: 'strpGroup',
            itemId: 'strpGroupItem',
            storeId: 'strpGroup',
            proxy: {
                type: 'rest',
                extraParams: {
                    dataMember: JSON.stringify(me.data)
                },
                url: apiUrl + 'master/LoadMemberBySchedule',
                reader: {
                    type: 'json'
                },
                writer: {
        ...
        var groupTree = Ext.create('Ext.tree.Panel', {
            ...
            rootVisible: false,
            viewConfig: {
                id: 'treeviewGroupMember',
                itemId: 'treeviewGroupMember',
                listeners: {
                    checkchange: {
                        fn: function (node, checked, eOpts) {
                            this.onSelectMemberChange(node, checked, eOpts);
                            MyApp.util.Utilities.userScheduleID = Global.authData.userEmpInfo.ACEMP_ID;
                            MyApp.util.Utilities.userScheduleName = Global.authData.userEmpInfo.ACEMP_NAME;
                            Ext.getCmp('txtSchedulePerson').setValue(Global.authData.userEmpInfo.ACEMP_NAME);
                            var userInformationId = [Global.authData.userEmpInfo.ACEMP_ID * 1];
                            Ext.getCmp('trpGroup').getView().node.cascadeBy(function (child) {
                                if (child.get('checked') === true & child.get('leaf') === true) {
                                    if (child.get('id') % 1 === 0 && userInformationId.indexOf(child.get('id') * 1) == -1) {
                                        userInformationId.push(child.get('id') * 1);
                                        MyApp.util.Utilities.userScheduleID += ',' + child.get('id') * 1;
                                        MyApp.util.Utilities.userScheduleName += ',' + child.get('text');
                                    }
                                }
                            });

                            Ext.getCmp('txtSchedulePerson').setValue(MyApp.util.Utilities.userScheduleName);
                            Ext.getCmp('txtSchedulePersonHidden').setValue(MyApp.util.Utilities.userScheduleID);
                        },
                        scope: this
                    }
                }
    ...
    onSelectMemberChange: function (node, checked, eOpts) {//console.log(node);
    ...
    getHasEditedConfig: function () {
        return {
            xtype: 'checkbox',
            id: 'chkScheduleEdited',
            boxLabel: '参加者も編集可能にする',
            handler: this.onEditedChange,
            scope: this
        };
    },
    ...
    getPersonHidden: function () {
        return {
            xtype: 'textfield',
            id: 'txtSchedulePersonHidden',
            itemId: this.id + '-person-group-item-hidden',
            name: 'person-hidden',
            width: 300,
            hidden: true,
            labelWidth: 28
 ...
 * @param {Object} config The config object
 */
Ext.define('Extensible.form.field.TreeDepartmentsRange', {
    extend: 'Ext.form.FieldContainer',
    alias: 'widget.extensible.treedepartmentsrangefield',
 ...
 * @param {Object} config The config object
 */
Ext.define('Extensible.form.field.SpaceLine', {
    extend: 'Ext.form.FieldContainer',
    alias: 'widget.extensible.spaceline',
 ...
 * @param {Object} config The config object
 */
Ext.define('Extensible.form.field.TreeRange', {
    extend: 'Ext.form.FieldContainer',
    alias: 'widget.extensible.treerangefield',
...
Ext.define('Extensible.form.field.DateRange', {
    ...
    getFieldConfigs: function () {
        ...
        return [
            me.getAllDayConfig(),
            me.getSpace1Config(),
            me.getStartDateSeparatorConfig(),
            me.getStartDateConfig(),
            me.getStartTimeConfig(),
            me.getDateSeparatorConfig(),
            me.getEndDateConfig(),
    ...
    getSpace1Config: function () {
        return {
            xtype: 'label',
            id: this.id + '-spaces1-label',
            text: "",
            width: 10
        };
    },

    getSpace2Config: function () {
        return {
            xtype: 'label',
            id: this.id + '-spaces2-label',
            text: "",
            width: 2
        };
    },
    getSpace3Config: function () {
        return {
            xtype: 'label',
            id: this.id + '-spaces3-label',
            text: "",
            width: 2
        };
    },
    ...
    getDateSeparatorConfig: function () {
        return {
            ...
            id: this.id + '-to-label',
            text: this.toText,
            margins: { top: 4, right: 5, bottom: 0, left: 9 }
        };
    },

    getStartDateSeparatorConfig: function () {
        return {
            xtype: 'label',
            id: this.id + '-from-label',
            text: this.fromText,
            margins: { top: 4, right: 5, bottom: 0, left: 9 }
        };
    },
     ...
     */
    //DoHuuHai 2014-06-11 get data for Date time
    getValue: function () {
        return [
            this.getDT('start'),
            this.getDT('end'),
 ...
 * @param {Object} config The config object
 */
Ext.define('Extensible.form.field.ComboRange', {
    extend: 'Ext.form.FieldContainer',
    alias: 'widget.extensible.comborangefield',
    ...
    roomText: 'Rooms',
    /**
     * @cfg {String} editedText
     * The text to display as the label for the all day checkbox (defaults to 'All day')
     */
    editedText: 'Has Edited',
    // private
    fieldLayout: {
        type: 'hbox',
        defaultMargins: { top: 0, right: 5, bottom: 0, left: 0 }
    ...
    getPlanType: function () {
        return {
            xtype: 'extensible.plantypecombo',
            itemId: this.id + '-plantype',
            name: 'plantypename',
            editable: false,
            width: 226,
            labelWidth: 80
    ...
    getRoom: function () {
        return {
            xtype: 'extensible.roomcombo',
            itemId: 'cboScheduleRooms',
            name: 'roomname',
            editable: false,
            width: 190,
            labelWidth: 40
    ...
    getStatus: function () {
        return {
            xtype: 'extensible.staffstatuscombo',
 ...
 * @param {Object} config The config object
 */
Ext.define('Extensible.calendar.form.field.DepartmentCombo', {
    extend: 'Ext.form.field.ComboBox',
    alias: 'widget.extensible.departmentcombo',
    ...
    getValueList: function () {
        ...
        Ext.Ajax.request({
            type: 'rest',
            async: false,
            url: apiUrl + 'schedule/GetSection',

            success: function (response) {//console.log(response.responseText);
                data = Ext.decode(response.responseText, true);//console.log(data);
            },
 ...
 * @param {Object} config The config object
 */
var planStore = Ext.create('Ext.data.ArrayStore', {
    fields: ['VALUE', 'TITLE'],
    idIndex: 0,
    proxy: {
        type: 'ajax',
        url: apiUrl + 'schedule/GetType',
        reader: {
            type: 'json'
        }
    }
});
Ext.define('Extensible.calendar.form.field.PlanTypeCombo', {
    extend: 'Ext.form.field.ComboBox',
    alias: 'widget.extensible.plantypecombo',
    id: 'cboSchedulePlanType',
    requires: ['Ext.data.ArrayStore'],
 ...
 * @param {Object} config The config object
 */
var staffStatusStore = Ext.create('Ext.data.ArrayStore', {
    fields: ['VALUE', 'TITLE'],
    idIndex: 0,
    proxy: {
        type: 'ajax',
        url: apiUrl + 'schedule/getStaffStatus',
        reader: {
            type: 'json'
        }
    }
});
Ext.define('Extensible.calendar.form.field.StaffStatusCombo', {
    extend: 'Ext.form.field.ComboBox',
    alias: 'widget.extensible.staffstatuscombo',
    id: 'cboScheduleStaffStatus',
    requires: ['Ext.data.ArrayStore'],
 ...
 * @param {Object} config The config object
 */
var roomStore = Ext.create('Ext.data.ArrayStore', {
    fields: ['VALUE', 'TITLE'],
    idIndex: 0,
    proxy: {
        type: 'ajax',
        url: apiUrl + 'schedule/GetRoom',
        reader: {
            type: 'json'
        }
    }
});
Ext.define('Extensible.calendar.form.field.RoomCombo', {
    extend: 'Ext.form.field.ComboBox',
    alias: 'widget.extensible.roomcombo',
...
Ext.define('Extensible.calendar.form.EventDetails', {
    ...
    // inherited docs
    loadRecord: function (rec) {//console.log(rec);
        var me = this,
            EventMappings = Extensible.calendar.data.EventMappings;
...
Ext.define('Extensible.calendar.form.EventWindow', {
    ...
    itemId: 'FrmEventWindow',
    // Locale configs
    titleTextAdd: 'スケジュールの新規作成 ',
    titleTextEdit: 'イベント編集',//Edit Event
    width: 681,
    height: 615,
    detailsLinkText: '詳細編集...',//edit details
    savingMessage: 'Saving changes...',
    deletingMessage: 'Deleting event...',
    saveButtonText: '保存',
    deleteButtonText: '削除',
    ...
    roomLabelText: '会議室',
    personLabelText: '社員',
    departmentLabelText: '部署',
    treeLabelText: 'プライバシー',
    treeDepartmentLabelText: 'Departments',
    treeDepartmentPersonLabelText: 'プロジェクト･ジョブ',
    notesLabelText: '備考',
    /**
     * @cfg {Number} startDay
     * The 0-based index for the day on which the calendar week begins (0=Sunday, which is the default)
    ...
    getFormItemConfigs: function () {
        ...
        //DoHuuHai: Type + Room + Status
        items.push({
            xtype: 'extensible.comborangefield',
            itemId: this.id + '-plancombo',
            name: 'plancombo',
            anchor: '100%'
        });
        //DoHuuHai: tree department + tree group
        items.push({
            xtype: 'extensible.treedepartmentgrouprangefield',
            itemId: this.id + '-treedepartmentrangefield',
            name: 'treedepartmentrangefield',
            anchor: '100%',
            data: MyApp.util.Utilities.userScheduleID
        });
        //DoHuuHai: public + private
        items.push({
            xtype: 'extensible.treerangefield',
            itemId: this.id + '-treegrouprangefield',
            anchor: '100%',
            labelWidth: 75,
            name: 'treegrouprangefield',
            fieldLabel: this.treeLabelText
        });
        //DoHuuHai: testArea
    ...
    initRefs: function () {
        ...
        this.comborangefield = this.down('#' + this.id + '-plancombo');
        this.treedepartmentgrouprangefield = this.down('#' + this.id + '-treedepartmentrangefield');
        this.treerangefield = this.down('#' + this.id + '-treegrouprangefield');
    ...
    show: function (o, animateTarget) {
        if (planStore.getCount() == 0) {
            planStore.load({
                'callback': function (a, b, c) {
                    if (c) {
                        Ext.getCmp('cboSchedulePlanType').setValue('1');
                    }
                }
            });
        }
        if (roomStore.getCount() == 0) {
            roomStore.load({
                'callback': function (a, b, c) {
                    if (c) {
                        this.insert(0, {'VALUE': 'NULL', 'TITLE': '---'});
                        Ext.getCmp('cboScheduleRooms').setValue('NULL');
                    }
                }
            });
        }
        if (staffStatusStore.getCount() == 0) {
            staffStatusStore.load({
                'callback': function (a, b, c) {
                    if (c) {
                        Ext.getCmp('cboScheduleStaffStatus').setValue('1');
                    }
                }
            });
        }
        ...
        Ext.getCmp('btnSchedudeSave').setDisabled(false);
        Ext.getCmp('btnSchedudeDelete').setDisabled(false);
        //checkbox 参加者も編集できる
        Ext.getCmp('projectJobCheckbox').setValue(true);
        Ext.getCmp('chkScheduleEdited').setValue(true);
        Ext.getCmp('txtScheduleTitle').setValue('');
        Ext.getCmp('cboScheduleDepartment').setValue(-1);
        Ext.getCmp('cboSchedulePlanType').setValue('1');
        Ext.getCmp('cboScheduleRooms').setValue('NULL');
        Ext.getCmp('txtScheduleDepartment').setValue('');
        Ext.getCmp('txtScheduleDepartmentHidden').setValue('');
        MyApp.util.Utilities.userScheduleID = Global.authData.userEmpInfo.ACEMP_ID * 1;
        MyApp.util.Utilities.userScheduleName = Global.authData.userEmpInfo.ACEMP_NAME;
        Ext.getCmp('txtSchedulePersonHidden').setValue(MyApp.util.Utilities.userScheduleID);
        Ext.getCmp('txtSchedulePerson').setValue(MyApp.util.Utilities.userScheduleName);
        //console.debug('sau khi gan', Ext.getCmp('txtSchedulePerson').getValue());
        Ext.getCmp('rdPublic').setValue(true);
        Ext.getCmp('rdPrivate').setValue(false);
        ...
        if (o.data) {
            ...
            me.setTitle(me.titleTextAdd);
            //DoHuuHai 2014/06/17    modify show time
            var start = Extensible.Date.add(o[EventMappings.StartDate.name], {hours: 8}),
            ...
            if (strpGroupLoading == false) {
                strpGroupLoading = true;
                Ext.getStore('strpGroup').load({
                    callback: function (a, b, c) {
                        if (c) {
                            strpGroupLoading = false;
                        }
                    }
                });
            }
        ...
        if (!rec.phantom) {
            ...
            Ext.Ajax.request({
                ...
                async: false,
                params: {
                    scheduleId: parseInt(rec.data.EventId)
                },
                url: apiUrl + 'schedule/GetScheduleByScheduleID',
                //Son33
                success: function (response) {
                    data = Ext.decode(response.responseText, true);
                    rec.data.ProjectName = data['schedule'][0]['JOB_ID'];

                    if (rec.data.IsAllDay) {
                        //If all day then display 8h am - 10pm but not save database
                        var day = new Date(rec.data[EventMappings.StartDate.name]);
                        day.setHours(0);
                        day.setMinutes(0);
                        rec.data[EventMappings.StartDate.name] = Extensible.Date.add(day, {hours: 8});

                        day = new Date(rec.data[EventMappings.EndDate.name]);
                        day.setHours(0);
                        day.setMinutes(0);
                        rec.data[EventMappings.EndDate.name] = Extensible.Date.add(day, {hours: 9});
                    }

                    rec.data.Url = (null != data['schedule'][0]['JOB_ID']) ? data['schedule'][0]['PROJECT_NAME'] + '/' + data['schedule'][0]['JOB_NAME'] : '';
                    rec.set('Title', data['schedule'][0]['TITLE']);
                    Ext.getCmp('txtScheduleNote').setValue(data['schedule'][0]['REMARKS']);
                    rec.data.Room = data['schedule'][0]['ROOM_ID'];
                    rec.data.ScheduleType = data['schedule'][0]['SCHEDULE_TYPE_ID'];
                    rec.data.PrivacySchedule = data['schedule'][0]['OPENED_FLAG'];
                    rec.data.EditEnable = data['schedule'][0]['LOCK_FLAG'];
                    var arrayMemberLength = data['scheduleStaff'].length;

                    if (0 < arrayMemberLength) {
                        for (i = 0; i < arrayMemberLength - 1; i++) {
                            MyApp.util.Utilities.userScheduleID = MyApp.util.Utilities.userScheduleID + parseInt(data['scheduleStaff'][i]['USERID']) + ',';
                            MyApp.util.Utilities.userScheduleName = MyApp.util.Utilities.userScheduleName + data['scheduleStaff'][i]['USERNAME'] + ',';
                        }
                        MyApp.util.Utilities.userScheduleID = MyApp.util.Utilities.userScheduleID + data['scheduleStaff'][arrayMemberLength - 1]['USERID'];
                        MyApp.util.Utilities.userScheduleName = MyApp.util.Utilities.userScheduleName + data['scheduleStaff'][arrayMemberLength - 1]['USERNAME'];
                    }
                    var interval = setInterval(function () {
                        if (strpGroupLoaded == true) {
                            window.clearInterval(interval);
                            var listScheduleEmployee = MyApp.util.Utilities.userScheduleID.toString().split(',');
                            if ((data['schedule'][0]['STAFF_ID'] != Global.authData.userEmpInfo.ACEMP_ID && rec.data.EditEnable == 1) || listScheduleEmployee.indexOf(Global.authData.userEmpInfo.ACEMP_ID) == -1) {
                                Ext.getCmp('btnSchedudeSave').setDisabled(true);
                                Ext.getCmp('btnSchedudeDelete').setDisabled(true);
                            }
                            Ext.getCmp('trpGroup').getView().node.cascadeBy(function (node) {
                                var index = listScheduleEmployee.indexOf(node.get('id'));
                                if (index != -1) {
                                    ...
                                    MyApp.app.common.checkParentNode(p);
                                    p.resumeEvents();
                                    me.expandParentsNode(node);
                                    listScheduleEmployee.splice(index, 1);
                                }
                            });
                        }
                    }, 10);
                    me.titleField.setValue(rec.data.Title);
                    Ext.getCmp('cboSchedulePlanType').setValue(rec.data.ScheduleType);
                    if (rec.data.Room == null) {
                        Ext.getCmp('cboScheduleRooms').setValue('NULL');
                    } else {///console.log(rec.data.Room);
                        Ext.getCmp('cboScheduleRooms').setValue(rec.data.Room);
                    }
                    Ext.getCmp('txtScheduleDepartment').setValue(rec.data.Url);
                    Ext.getCmp('txtScheduleDepartmentHidden').setValue(rec.data.ProjectName);
                    //checkbox 参加者も編集できる
                    Ext.getCmp('chkScheduleEdited').setValue(!parseInt(rec.data.EditEnable));
                    Ext.getCmp('txtSchedulePerson').setValue(MyApp.util.Utilities.userScheduleName);
                    Ext.getCmp('txtSchedulePersonHidden').setValue(MyApp.util.Utilities.userScheduleID);
                    //check privacy
                    if (1 == parseInt(rec.data.PrivacySchedule)) {
                        Ext.getCmp('rdPublic').setValue(true);
                        Ext.getCmp('rdPrivate').setValue(false);
                    } else {
                        Ext.getCmp('rdPrivate').setValue(true);
                        Ext.getCmp('rdPublic').setValue(false);
                    }
                },
        ...
        // Using setValue() results in dirty fields, so we reset the field state
        // after loading the form so that the current values are the "original" values
        //DoHuuHai hình như chỗ này reset thì phải
    ...
    //DoHuuHai: cho nay cap nhat data cho việc add va edit
    updateRecord: function (record, keepEditing) {
        ...
        var scheduleDates = this.dateRangeField.getValue(),
            allday = obj[EventMappings.IsAllDay.name] = scheduleDates[2];
        //re-get date
        // Clear times for all day events so that they are stored consistently
        var startDate = allday ? Extensible.Date.clearTime(scheduleDates[0]) : scheduleDates[0],
            endDate = allday ? Extensible.Date.clearTime(scheduleDates[1]) : scheduleDates[1],
            singleDayDurationConfig = { days: 1 };
        ...
        //obj[EventMappings.StartDate.name] = startDate;
        if (allday)
            startDate = Extensible.Date.add(startDate, {days: 1});
        obj[EventMappings.StartDate.name] = allday ? Extensible.Date.add(startDate, {hours: -24}) : startDate;

        // If the event is all day, calculate the end date as midnight of the day after the end
        // date minus 1 unit based on the EventModel resolution, e.g. 23:59:00 on the end date
        endDate = allday ?
            Extensible.Date.add(endDate, singleDayDurationConfig) : endDate;
        obj[EventMappings.EndDate.name] = allday ? Extensible.Date.add(Extensible.Date.add(endDate, {hours: -24}), {minutes: 1}) : endDate;


        //console.log(obj[EventMappings.EndDate.name]);return;
        ...
        //DoHuuHai Get data from comborangefield
        var typeRoomStatus = this.comborangefield.items.items;
        obj[EventMappings.ScheduleType.name] = typeRoomStatus[0].getValue();
        obj[EventMappings.Room.name] = typeRoomStatus[2].getValue();
        obj[EventMappings.ScheduleStatus.name] = typeRoomStatus[4].getValue();

        var getTreedepartmentgrouprangefield = this.treedepartmentgrouprangefield.items.items;
        obj[EventMappings.ProjectName.name] = getTreedepartmentgrouprangefield[0].items.items[5].items.items[0].getValue();
        obj[EventMappings.EditEnable.name] = (Ext.getCmp('chkScheduleEdited').getValue()) ? 0 : 1;
        obj[EventMappings.MemberSchedule.name] = getTreedepartmentgrouprangefield[2].items.items[5].items.items[0].getValue();
        var getPrivacySchedule = this.treerangefield.items.items;

        obj[EventMappings.PrivacySchedule.name] = (getPrivacySchedule[0].items.items[0].items.items[0].getValue()) ? 1 : 0;
        record.beginEdit();
        record.set(obj);
    ...
    onSave: function () {
        ...
        //console.log(me.activeRecord.data.Title);return;
        if ('' === me.activeRecord.data.Title.trim()) {//console.log(Ext.MessageCommon);
            Ext.MessageBox.show({
                title: MessageCommon.ScheduleNoteTitle,
                msg: MessageCommon.MissingScheduleTitle,
                icon: Ext.MessageBox.WARNING,
                buttons: Ext.Msg.OK
            });
            //Ext.getCmp[this.id + '-title'].focus(true, 1);
            return;
        }
        /*else if(/[`~,.<>;':"/[\]|{}()=_+-]/.test(me.activeRecord.data.Title)){
         Ext.MessageBox.show({
         title: MessageCommon.ScheduleNoteTitle,
         msg: MessageCommon.InvalidScheduleTitle,
         icon: Ext.MessageBox.WARNING,
         buttons: Ext.Msg.OK
         });
         return;
...
Ext.define('Extensible.calendar.view.AbstractCalendar', {
    ...
    // private
    initComponent: function () {
        this.setStartDate(MyApp.util.Utilities.dateSelected);
    ...
    reloadStore: function (o) {
        ...
        //this.store.getProxy().setExtraParam("currDate", o.startDate);
        this.store.load(o);
    },
    ...
    refresh: function (reloadData) {
        //this.startDate = new Date(MyApp.util.Utilities.dateSelected);
...
//        }
    ...
    setStartDate: function (start, /*private*/reload) {
        var me = this;
        var formatDate = Ext.Date.format(start, 'Y-m-d');
        Extensible.log('setStartDate (base) ' + formatDate);
        //DoHuuHai 2014/05/28

        //Ext.getCmp(this.id + '-tb-jump-dt').setValue(MyApp.util.Utilities.dateSelected);
        //this.store.getProxy().setExtraParam("currDate", MyApp.util.Utilities.dateSelected);
        //this.store.getProxy().setExtraParam("listEmployee", JSON.stringify(MyApp.util.Utilities.listEmployee));
    ...
    moveTo: function (dt, /*private*/reload) {
        var formatDates = Ext.Date.format(dt, 'Y-m-d');
        Extensible.log('setStartDate (base) ' + formatDates);
        //DoHuuHai 2014/05/28
        //if (1==MyApp.util.Utilities.goTime){
            MyApp.util.Utilities.dateSelected = formatDates;
            MyApp.app.common.callbackSchedule(2, '', false, '', MyApp.util.Utilities.activeViewValue);
        //}
        //Ext.getCmp(this.id + '-tb-jump-dt').setValue(MyApp.util.Utilities.dateSelected);
        Ext.getStore('StScheduleStoreId').getProxy().setExtraParam("currDate", formatDates);
        if (Ext.isDate(dt)) {
    ...
    moveMonths: function (value, /*private*/reload) {
        //if (1==MyApp.util.Utilities.goTime){
            MyApp.util.Utilities.dateSelected = Extensible.Date.add(this.startDate, {months: value});
    ...
    moveWeeks: function (value, /*private*/reload) {
        //if (1==MyApp.util.Utilities.goTime){
            MyApp.util.Utilities.dateSelected = Extensible.Date.add(this.startDate, {days: value * 7});
            console.log("   chay select ");
            MyApp.app.common.callbackSchedule(2, '', false, '', MyApp.util.Utilities.activeViewValue);
        //}
    ...
    moveDays: function (value, /*private*/reload) {
        //if (1==MyApp.util.Utilities.goTime){
            MyApp.util.Utilities.dateSelected = Extensible.Date.add(this.startDate, {days: value});
            console.log(Extensible.Date.add(this.startDate, {days: value}) + "   chay select ");
            MyApp.app.common.callbackSchedule(2, '', false, '', MyApp.util.Utilities.activeViewValue);
        //}
    ...
    moveToday: function (/*private*/reload) {
        //if (1==MyApp.util.Utilities.goTime){
            MyApp.util.Utilities.dateSelected = new Date();
            console.log(new Date() + "   chay select ");
            MyApp.app.common.callbackSchedule(2, '', false, '', MyApp.util.Utilities.activeViewValue);
       // }
    ...
    //DoHuuHai 2014-06-10 Add or Edit
    // private
     ...
     */
    showEventEditor: function (rec, animateTarget) {//console.log(rec);return;
    ...
    // private
    save: function (infoSchedule, addOrEdit) {
        var loadMask = new Ext.LoadMask(this.editWin, {msg: '処理が完成するまでお願いします。'});
        loadMask.show();
        var me = this;
        //setTimeout(function () {
            var infoScheduleAdd = MyApp.app.common.cloneObj(infoSchedule);
            var roomCheck = infoSchedule.Room * 1;
            var successful = true;

            var startDateTime = infoSchedule.StartDate;
            var startDateSchedule = MyApp.app.common.getScheduleToday(startDateTime);
            var startTimeSchedule = MyApp.app.common.getTime(startDateTime);
            var startTime = startDateSchedule + " " + startTimeSchedule;

            var endDateTime = infoSchedule.EndDate;
            var endDateSchedule = MyApp.app.common.getScheduleToday(endDateTime);
            var endTimeSchedule = MyApp.app.common.getTime(endDateTime);
            var endTime = endDateSchedule + " " + endTimeSchedule;
            var messageCheckSchedule = '';
            var result_Room = true;

            infoScheduleAdd.StartDate = startTime;
            infoScheduleAdd.EndDate = endTime;

            if (2 == addOrEdit) {
                successful = false;
                Ext.Msg.confirm(MessageCommon.ScheduleDeleteSuccessTitle, 'このスケジュールを削除ｓじたいですか？', function (btn, text) {
                    if (btn === 'yes') {
                        successful = true;
                        Ext.Ajax.request({
                            type: 'rest',
                            ...
                            params: {
                                arrayScheduleInfo: JSON.stringify(infoScheduleAdd)
                            },
                            success: function (response) {
                                loadMask.hide();
                            },
                            ...
                            failure: function (fp, o) {
                                successful = false;
                                Ext.MessageBox.show({
                                    title: MessageCommon.ScheduleErrorTitle,
                                    msg: MessageCommon.ScheduleDeleteError,
                ...
                if (0 < roomCheck) {
                    var start_date = '',
                        end_date = '',
                        is_all_days = infoScheduleAdd['IsAllDay'];
                    if (!is_all_days) {
                        start_date = infoScheduleAdd['StartDate'];
                        end_date = infoScheduleAdd['EndDate'];
                    }
                    ...
                    if ('' == start_date || '' == end_date) {
                        result_Room = false;
                        messageCheckSchedule = messageCheckSchedule + "■会議室チェックに失敗しました<br/>・開始日時、終了日時が不明";
                        loadMask.hide();
                        Ext.MessageBox.show({
                            title: MessageCommon.ScheduleNoteTitle,
                            msg: messageCheckSchedule,
                    ...
                    //終日スケジュールの場合のチェック対象時間帯
                    if (is_all_days) {
                        //var startDateSchedule = MyApp.app.common.getScheduleToday(start_date);
                        start_date = start_date.substr(0, 10) +'00:00:00';
                        end_date = Extensible.Date.add(start_date, {days: 1});
                    }
                    ...
                    Ext.Ajax.request({
                        ...
                        async: false,
                        params: {
                            scheduleid: JSON.stringify(infoScheduleAdd['EventId']),
                            room: JSON.stringify(infoScheduleAdd['Room']),
                            startdate: JSON.stringify(start_date),
                            enddate: JSON.stringify(end_date)
                        },
                        url: apiUrl + 'schedule/CheckRoom',
                        success: function (response) {
                            messageRoom = Ext.decode(response.responseText, true);
                        },
                        failure: function (fp, o) {
                            result_Room = false;
                            successful = false;
                    ...
                    //==============================================================================================
                    //console.log(messageRoom);return;
                    if ('' != messageRoom) {
                        result_Room = false;
                        messageCheckSchedule = messageCheckSchedule + messageRoom;
                    }
                    if (!result_Room) {//console.log('bao loi check room');
                        loadMask.hide();
                        Ext.MessageBox.show({
                            title: MessageCommon.ScheduleNoteTitle,
                            msg: messageCheckSchedule,
                ...
                if (result_Room) {
                    if (0 == addOrEdit) {
                        Ext.Ajax.request({
                            type: 'rest',
                            async: false,
                            url: apiUrl + 'schedule/AddSchedule',
                            params: {
                                arrayScheduleInfo: JSON.stringify(infoScheduleAdd)
                            },
                            success: function (response) {
                                ...
                                if (returnValue.success == false) {
                                    ...
                                    Ext.MessageBox.show({
                                        title: MessageCommon.ScheduleErrorTitle,
                                        msg: MessageCommon.InsertNullScheduleError,
                                        icon: Ext.MessageBox.ERROR,
                                        buttons: Ext.Msg.OK
                                    });
                                    return;
                            ...
                            failure: function (fp, o) {
                                ...
                                Ext.MessageBox.show({
                                    title: MessageCommon.ScheduleErrorTitle,
                                    msg: MessageCommon.ScheduleError,
                                    icon: Ext.MessageBox.ERROR,
                                    buttons: Ext.Msg.OK
                                });
                                return;
                        ...
                        Ext.Ajax.request({
                            type: 'rest',
                            async: false,
                            url: apiUrl + 'schedule/EditSchedule',
                            params: {
                                arrayScheduleInfo: JSON.stringify(infoScheduleAdd)
                            },
                            success: function (response) {
                                returnValue = Ext.decode(response.responseText, true);//console.log(returnValue);
                                if (returnValue.success == false) {
                                    successful = false;
                                    Ext.MessageBox.show({
                                        title: MessageCommon.ScheduleErrorTitle,
                                        msg: MessageCommon.EditNullScheduleError,
                                        icon: Ext.MessageBox.ERROR,
                                        buttons: Ext.Msg.OK
                                    });
                                    return;
                            ...
                            failure: function (fp, o) {
                                ...
                                Ext.MessageBox.show({
                                    title: MessageCommon.ScheduleErrorTitle,
                                    msg: MessageCommon.EditScheduleError,
                                    icon: Ext.MessageBox.ERROR,
                                    buttons: Ext.Msg.OK
                                });
                                return;
...
// private
    onEventEditorAdd: function (form, rec) {
        this.newRecord = rec;//console.log(rec.data);return;

        if (this.store.indexOf(rec) === -1) {
            this.store.add(rec);
        }
        //console.log('data for adding schedule', rec.data);
        this.save(rec.data, 0);
        this.fireEvent('eventadd', this, rec);
    },

// private
    onEventEditorUpdate: function (form, rec) {//console.log(rec.data);return;
        this.save(rec.data, 1);
...
// private -- called from subclasses
    onDayClick: function (dt, ad, el) {//DoHuuHai Add new Event day roi
        if (this.readOnly === true || 0 < Extensible.Date.diffDays(dt, new Date())) {
            return;
        }
        //console.log("Chay chỗ này "+dt);
        if (this.fireEvent('dayclick', this, Ext.Date.clone(dt), ad, el) !== false) {
            var M = Extensible.calendar.data.EventMappings,
                data = {};
            if (null === el)
                data[M.StartDate.name] = Extensible.Date.add(dt, {hours: -8});
            else
...
// private
    onMoveEvent: function (menu, rec, newStartDate) {//console.log(newStartDate);
        this.menuActive = false;
...
// private
    ...
    onClick: function (e, t) {
        ...
        if (el) {
            var id = me.getEventIdFromEl(el),checkOpened = true,
                rec = me.getEventRecord(id);

            Ext.Ajax.request({
                type: 'rest',
                async: false,
                url: apiUrl + 'schedule/CheckOpened',
                params: {
                    scheduleid: JSON.stringify(rec.data.EventId)
                },
                success: function (response) {//console.log(response.responseText);
                    data = Ext.decode(response.responseText, true);
                    if (0<data)
            ...
            //console.log(checkOpened);return;
            if (rec && me.fireEvent('eventclick', me, rec, el) !== false) {
                if (me.readOnly !== true && checkOpened && 0 <= Extensible.Date.diffDays(new Date(), rec.data.EndDate)) {
        ...
        if (t) {
            if (t.id && t.id.indexOf(this.dayElIdDelimiter) > -1) {
                ...
                if (!rel || dt !== relDate) {
                    ...
                    this.fireEvent('day' + type, this, Ext.Date.parseDate(dt, "Ymd"), el);//DoHuuHai: phat sinh  trên luoi voi mouse
                }
            }
        }
    },
...
Ext.define('Extensible.calendar.view.Month', {
    ...
    getTemplateEventData: function (evtData) {
        ...
        templateData.Staff = (0==evtData.Privates)?'': evtData.Staffs;
        templateData.TitleToolTip = (0==evtData.Privates)? ((evtData[M.IsAllDay.name] ? Ext.Date.format(evtData[M.StartDate.name], fdatetime) : Ext.Date.format(evtData[M.StartDate.name], fmt)) +  '- ' + (evtData[M.IsAllDay.name] ? Ext.Date.format(evtData[M.EndDate.name], fdatetime) : Ext.Date.format(evtData[M.EndDate.name], fmt))) :(evtData.StaffsNameFull + (evtData[M.IsAllDay.name] ? Ext.Date.format(evtData[M.StartDate.name], fdatetime) : Ext.Date.format(evtData[M.StartDate.name], fmt)) +  '- ' + (evtData[M.IsAllDay.name] ? Ext.Date.format(evtData[M.EndDate.name], fdatetime) : Ext.Date.format(evtData[M.EndDate.name], fmt)) +
            ' '+ (!title || title.length === 0 ? this.defaultEventTitleText : title));
        templateData.Title = (0==evtData.Privates)? (evtData[M.IsAllDay.name] ? Ext.Date.format(evtData[M.StartDate.name], fdatetime) : Ext.Date.format(evtData[M.StartDate.name], fmt)) : (evtData.StaffsName + (evtData[M.IsAllDay.name] ? Ext.Date.format(evtData[M.StartDate.name], fdatetime) : Ext.Date.format(evtData[M.StartDate.name], fmt)) +
            ' '+ (!title || title.length === 0 ? this.defaultEventTitleText : title));
        /* if(3==MyApp.util.Utilities.activeViewValue)
         templateData.Title = (evtData[M.IsAllDay.name] ? '' : '') +
         (!title || title.length === 0 ? this.defaultEventTitleText : title);*/
        //DoHuuHai: display grid
    ...
    onClick: function (e, t) {
        ...
        if (e.addNew) {
            ...
            day.setHours(8, 0, 0);
            //console.log(day);
            this.onDayClick(day, false, null);
            var interval = setInterval(function () {
                if (clearStore) {
 ...
 * Normally you should not need to use this class directly -- instead you should use {@link  * Extensible.calendar.view.Day DayView} which aggregates this class and the {@link * Extensible.calendar.view.DayHeader DayHeaderView} into the single unified view
 * presented by {@link Extensible.calendar.CalendarPanel CalendarPanel}.
 * @constructor
 * @param {Object} config The config object
 */
...
//                '</tpl>'
    ...
    getTemplateEventData: function (evtData) {
        ...
        data.Staff = (0==evtData.Privates)?'':evtData.Staffs;
        data.TitleToolTip = (0==evtData.Privates)? ((evtData[M.IsAllDay.name] ? Ext.Date.format(evtData[M.StartDate.name], fdatetime) : Ext.Date.format(evtData[M.StartDate.name], fmt)) +  '- ' + (evtData[M.IsAllDay.name] ? Ext.Date.format(evtData[M.EndDate.name], fdatetime) : Ext.Date.format(evtData[M.EndDate.name], fmt))) :(evtData.StaffsNameFull + (evtData[M.IsAllDay.name] ? Ext.Date.format(evtData[M.StartDate.name], fdatetime) : Ext.Date.format(evtData[M.StartDate.name], fmt)) +  '- ' + (evtData[M.IsAllDay.name] ? Ext.Date.format(evtData[M.EndDate.name], fdatetime) : Ext.Date.format(evtData[M.EndDate.name], fmt)) +
            ' '+ (!title || title.length === 0 ? this.defaultEventTitleText : title));
        ...
        data.Title = (0==evtData.Privates)? (evtData[M.IsAllDay.name] ? Ext.Date.format(evtData[M.StartDate.name], fdatetime) : Ext.Date.format(evtData[M.StartDate.name], fmt)) : (evtData.StaffsName + (evtData[M.IsAllDay.name] ? Ext.Date.format(evtData[M.StartDate.name], fdatetime) : Ext.Date.format(evtData[M.StartDate.name], fmt)) +
            ' '+ (!title || title.length === 0 ? this.defaultEventTitleText : title));
...
Ext.define('Extensible.calendar.CalendarPanel', {
    ...
    initComponent: function () {
        ...
        if (MyApp.util.Utilities.dateSelected.toString().length > 10) {
            var dd = MyApp.util.Utilities.dateSelected.getDate();
            var mm = MyApp.util.Utilities.dateSelected.getMonth() + 1; //January is 0!
            var yyyy = MyApp.util.Utilities.dateSelected.getFullYear();
        ...
        //console.log("Đặt lai ngay ban dau load:      "+startDate);
        this.viewCount = 0;
        ...
        var StSegment = Ext.create('Ext.data.Store', {
            fields: ['abbr', 'name'],
            data: [
                {"abbr": "10", "name": "10 分"},
                {"abbr": "15", "name": "15 分"},
        ...
        if (this.showNavJump) {
            ...
            this.tbar.items.push({itemId: 'dateItemId', id: this.id + '-tb-jump-dt', xtype: 'datefield', value: new Date(MyApp.util.Utilities.dateSelected), format: 'Y/m/d', width: 120, editable: false, showToday: false, startDay: this.startDay,
                listeners: {
                    'change': {
                        fn: function () {
                            this.onJumpClick();
            ...
            this.tbar.items.push({id: this.id + '-tb-emptyLabel-dt', xtype: 'label', text: '', labelWidth: 150});
            this.tbar.items.push({itemId: 'segmentItemId', id: this.id + '-tb-combo-dt', xtype: 'combo', disabled: (1 < MyApp.util.Utilities.activeViewValue) ? true : false, valueField: 'abbr', editable: false, displayField: 'name', value: '30 分', store: StSegment,
                width: 120, listeners: {
                    'change': {
                        fn: function (dt) {
                            var segmentValue = Ext.getCmp(this.id + '-tb-combo-dt').getValue();
                            if (MyApp.util.Utilities.segmentTime !== segmentValue) {
                                MyApp.util.Utilities.segmentTime = segmentValue;
                                MyApp.app.common.callbackSchedule(MyApp.util.Utilities.segmentTime, Ext.getCmp(this.id + '-tb-combo-dt').getItemId(), Ext.getCmp(this.id + '-tb-jump-dt').getValue(), Ext.getCmp(this.id + '-tb-jump-dt').getItemId(), MyApp.util.Utilities.activeViewValue);
                            }
                        },
                        scope: this
                    }
    ...
    onTodayClick: function () {
        if (Ext.getCmp(this.id + '-tb-jump-dt').getValue())
            Ext.getCmp(this.id + '-tb-jump-dt').setValue('');
    ...
    onJumpClick: function () {
        ...
        if (dt !== '') {
            MyApp.util.Utilities.dateSelected = MyApp.app.common.getScheduleToday(dt);
            //if (1==MyApp.util.Utilities.goTime){
                MyApp.app.common.callbackSchedule(2, '', false, '', MyApp.util.Utilities.activeViewValue);
            //}
    ...
    onPrevClick: function () {
		var thisButton = Ext.getCmp(this.id + '-tb-prev');
    ...
    onNextClick: function () {
        var thisButton = Ext.getCmp(this.id + '-tb-next');
		thisButton.setDisabled(true);
        setTimeout(
            function(){
                thisButton.setDisabled(false);
    ...
    onDayNavClick: function () {
        MyApp.util.Utilities.activeViewValue = 0;
        Ext.getCmp(this.id + '-tb-combo-dt').enable();
        //this.setActiveView(this.id + '-day');
        MyApp.app.common.callbackSchedule(0, Ext.getCmp(this.id + '-tb-combo-dt').getItemId(), Ext.getCmp(this.id + '-tb-jump-dt').getValue(), Ext.getCmp(this.id + '-tb-jump-dt').getItemId(), MyApp.util.Utilities.activeViewValue);
    },
    ...
    onWeekNavClick: function () {
        MyApp.util.Utilities.activeViewValue = 1;
        Ext.getCmp(this.id + '-tb-combo-dt').enable();
        //this.setActiveView(this.id + '-week');
        MyApp.app.common.callbackSchedule(1, Ext.getCmp(this.id + '-tb-combo-dt').getItemId(), Ext.getCmp(this.id + '-tb-jump-dt').getValue(), Ext.getCmp(this.id + '-tb-jump-dt').getItemId(), MyApp.util.Utilities.activeViewValue);
    },
    ...
    onMonthNavClick: function () {
        MyApp.util.Utilities.activeViewValue = 2;
        Ext.getCmp(this.id + '-tb-combo-dt').disable();
        //this.setActiveView(this.id + '-month');
        MyApp.app.common.callbackSchedule(2, Ext.getCmp(this.id + '-tb-combo-dt').getItemId(), Ext.getCmp(this.id + '-tb-jump-dt').getValue(), Ext.getCmp(this.id + '-tb-jump-dt').getItemId(), MyApp.util.Utilities.activeViewValue);
    },
